import { Black, CapsStyle, DisplayObject, FontStyle, FontWeight, Graphics, TextField, Vector, Rectangle } from "black-engine";
...
export class TileRaycaster2dDemo extends DisplayObject {
  ...
  _getTile(x, y) {
    ...
    const tile = { x, y, type: TILE_TYPE.EMPTY };
  ...
  _setTileCollided(x, y, collideType = null) {
    ...
    if (collideType === null) {
      collideType = (tile.type === TILE_TYPE.WALL) ? TILE_TYPE.WALL_COLLIDE : TILE_TYPE.EMPTY_COLLIDE
    }
  ...
  _setAllCollidesConnected() {
    for (let i = 0; i < this.tiles.length; i++) {
      ...
      if (this.tiles[i].type === TILE_TYPE.EMPTY_COLLIDE) {
  ...
  _clearCollision() {
    for (let i = 0; i < this.tiles.length; i++) {
      ...
      if (tile.type === TILE_TYPE.WALL_COLLIDE || tile.type === TILE_TYPE.WALL) {
  ...
  _initWallsAndPositions() {
    ...
    for (let i = 0; i < initData.walls.length; i++) {
      ...
      this.tiles.push({ x, y, type: TILE_TYPE.WALL });
    }
  }
  ...
  _createGridView() {
    ...
    gridViewContainer.on('pointerDown', (_, pointerInfo) => {
      ...
      isPressed = true;
      isClearing = (tile.type === TILE_TYPE.WALL || tile.type === TILE_TYPE.WALL_COLLIDE);
    });
    ...
    gridViewContainer.onUpdate = () => {
      ...
      if (isClearing && (tile.type === TILE_TYPE.WALL || tile.type === TILE_TYPE.WALL_COLLIDE)) {
...
TILE_COLORS[TILE_TYPE.CONNECT] = 0x04356c;
TILE_COLORS[TILE_TYPE.CONNECT_LAST] = 0x2060a9;
TILE_COLORS[TILE_TYPE.CONNECT_END] = 0x1ca8d9;
